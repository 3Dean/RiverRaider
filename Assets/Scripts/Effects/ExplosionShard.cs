...
public class ExplosionShard : MonoBehaviour
{
    ...
    [SerializeField] private float stickingVelocityThreshold = 0.2f;
    [SerializeField] private bool canStickToTerrain = true;
    [SerializeField] private float minimumFallTime = 2f;
    ...
    /// <summary>
    /// Initialize the shard with generic MonoBehaviour parent (for null safety)
    /// </summary>
    public void Initialize(MonoBehaviour parent, AudioClip[] sounds)
    {
        parentExplosion = parent;
        collisionSounds = sounds;
    ...
    private void HandleCollisionPhysics(Collision collision, float impactVelocity)
    {
        ...
        float energyLoss = Mathf.Clamp01(impactVelocity / 20f) * 0.3f;
    ...
    private void CreateSimpleSparks(Vector3 position, Vector3 normal)
    {
        ...
        Material sparksMaterial = Resources.Load<Material>("Sparks");
        if (sparksMaterial == null)
        {
            sparksMaterial = Resources.Load<Material>("Hit");
        }
        if (sparksMaterial != null)
        {
    ...
    private void CreateSimpleFireTrail()
    {
        ...
        var renderer = particles.GetComponent<ParticleSystemRenderer>();
        Material fireMaterial = Resources.Load<Material>("Fire");
        if (fireMaterial == null)
        {
            fireMaterial = Resources.Load<Material>("Hit");
        }
        if (fireMaterial != null)
        {
    ...
    private void DestroyImmediate()
    {
        ...
        // Notify parent explosion using reflection to call OnShardDestroyed
        if (parentExplosion != null)
        {
            var method = parentExplosion.GetType().GetMethod("OnShardDestroyed");
            if (method != null)
            {
                method.Invoke(parentExplosion, null);
            }
        }
